...
class Field(val rules: Rules = Rules.Standard, onIncorrectInitialMove: (MoveInfo, Boolean, Int) -> Unit = { _, _, _ -> }) {
    ...
    fun dump(printNumbers: Boolean = true, padding: Int = Int.MAX_VALUE, printCoordinates: Boolean = true, debugInfo: Boolean = false): String {
        var minX = width
        var maxX = OFFSET
        var minY = height
        var maxY = OFFSET
        ...
        val positionToMoveResult = moveResults.associateBy { it.position }
